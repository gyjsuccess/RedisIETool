...
public class VoteTest {
	...
	private static Logger log = LoggerFactory.getLogger(VoteTest.class);
	...
	public static void main(String[] args) {
		...
		List<String> ipPorts = jedis.lpopList("ActiveProxy", 5);
		ExecutorService eService = Executors.newFixedThreadPool(ipPorts.size());
		for(String ipPort : ipPorts){
			String[] arr = ipPort.split(":");
			String ip = arr[0];
			String port = arr[1];
			eService.submit(new VoteThread(url, ip, port));
		}
	}
	...
	public static void vote(String url, String ip, String port, String xIp) {	
		try {
			...
			addHeaders(httpRequest, xIp == null ? ip : xIp);
	...
	private static void addHeaders(HttpUriRequest httpGet, String ip) {
        ...
        try {
			while(xIp == null){
				while(true){
	        		xIp = jedis.rpop(REDIS_KEY_NAME, 5);
	        		if(xIp == null){
	        			Thread.sleep(100 * 2);
	        		}else{
	        			break;
			...
			jedis.lpush(REDIS_KEY_NAME, xIp, 5);
			log.info("xIp is :{}", xIp);
		} catch (Exception e) {
			log.error(e.getMessage());
		}
        httpGet.setHeader("X-Forwarded-For", xIp.split(":")[0]);
